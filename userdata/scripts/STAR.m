...
cstPat = matRadJoana_replicateCstFromDoseCube(cstPat, pln, doseCubePat, ...
...
% OR
load("trial_stf_dij_19beams_grid10.mat")

% Perform Fluence Optimization
resultGUIInit = matRad_fluenceOptimization(dij, [cst(1, :); cst(10, :)], pln);
...
doseCubeCurrent = resultGUICurrent.physicalDose; 
wCurrent = resultGUICurrent.w; 
clear resultGUICurrent

qiCurrent = matRadJoana_calcQualityIndicators(cst, pln, doseCubeCurrent, [], []);

min = qiCurrent(10).D_98;
max = qiCurrent(10).D_2;

if qiCurrent(ixPTV).D_98 > minDoseGoalPTV && qiCurrent (ixPTV).D_2 < maxDoseGoalPTV
    % qiInit(ixPTV).D_98 > minDoseGoalPTV && qiInit(ixPTV).D_2 < maxDoseGoalPTV
    sprintf('We did it!!')
else
    sprintf('We have to do the other way:(((((!!')
    sprintf(['Ups ', num2str(qiCurrent(ixPTV).D_98), ' and ', num2str(qiCurrent(ixPTV).D_2)])
end
...
matRadJoana_ShowSliceFast(ct, cst, doseCubePat); title('doseCubePat')
matRadJoana_ShowSliceFast(ct, cst, doseCubeCurrent); title('doseCubeCurrent')
matRadJoana_ShowSliceFast(ct, cst, [doseCubePat - doseCubeCurrent]); title('doseCubePat - doseCubeCurrent')
