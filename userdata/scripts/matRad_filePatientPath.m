function [targetPath, patientFolder] = matRad_filePatientPath(patientID, searchFolder, searchFile)
% matRad_filePatientPath - Locate patient data folders/files flexibly
%
% USAGE:
%   [targetPath, patientFolder, targetName] = matRad_filePatientPath(patientID)
%       -> finds patient folder and its RTFiles subfolder
%
%   [targetPath, patientFolder, targetName] = matRad_filePatientPath(patientID, searchFolder)
%       -> finds patient subfolder containing 'searchFolder' in its name
%
%   [targetPath, patientFolder, targetName] = matRad_filePatientPath(patientID, searchFolder, searchFile)
%       -> finds specific file (searchFile) inside the subfolder matching 'searchFolder'
%
%   [targetPath, patientFolder, targetName] = matRad_filePatientPath(patientID, [], searchFile)
