function [stf, stf_mb, stf_sb] = matRadJoana_generateStfOneEL(ct, cst, pln)
...
for i = 1:size(stf.ray,2)
    stf.ray(i).energy = stf_sb.ray(1).energy;
    stf.ray(i).rangeShifter = stf_sb.ray(1).rangeShifter;
    stf.ray(i).focusIx = stf_sb.ray(1).focusIx;
end
